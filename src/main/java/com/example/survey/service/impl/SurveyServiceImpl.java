...
//======================↓delete相關↓============================================================
	...
	public ResponseSurvey deleteSurvey(List<Integer> id) {
		...
		for(Survey delItem:delTarget) {
			if(!delItem.getStatus_is_published()||delItem.getStatus_is_published()&&LocalDate.now().isBefore(delItem.getStart_date())) {
...
//		如果預設日期沒填上 自動生成搜尋的時間範圍  因為null值不會列入搜尋時間的範圍
		...
		List<Survey> svList = surveydao.findByTitleContainingAndStartdateGreaterThanEqualAndEnddateLessThanEqual(title, start_date, end_date);
		//超長JPA語法 吐了
...
//		如果預設日期沒填上 自動生成搜尋的時間範圍  因為null值不會列入搜尋時間的範圍
		...
		if(isAll) {
			...
			svList=surveydao.findByTitleContainingAndStartdateGreaterThanEqualAndEnddateLessThanEqual(title, startdate, enddate);
		}else {
			svList=surveydao.findByTitleContainingAndStartdateGreaterThanEqualAndEnddateLessThanEqualAndPublishedTrue(title, startdate, enddate);
			//前台搜尋 看不到未發布問卷
			}
